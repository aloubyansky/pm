...
public class ConfigArranger {
    ...
    List<ResolvedFeature> orderFeatures(ConfigModelStack config) throws ProvisioningException {
        specFeatures = config.specFeatures;
        features = config.features;
        try {
            doOrder(config.rt);
        } catch (ProvisioningException e) {
            throw new ProvisioningException(Errors.failedToBuildConfigSpec(config.id.getModel(), config.id.getName()), e);
        }
